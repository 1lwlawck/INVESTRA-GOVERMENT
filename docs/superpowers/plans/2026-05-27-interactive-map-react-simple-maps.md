# Interactive Map react-simple-maps Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the dashboard's approximate manual SVG map with a real `react-simple-maps` choropleth using Indonesia's 38-province GeoJSON.

**Architecture:** Keep `InteractiveMap.tsx` as the dashboard map entry point. Replace manual island paths, coordinate dots, and pan handlers with `ComposableMap`, `ZoomableGroup`, `Geographies`, and `Geography`; keep existing API data flow, tooltip, legend, loading state, empty state, and cluster stat cards.

**Tech Stack:** React 18, TypeScript, Vite, Tailwind CSS, react-simple-maps v3, GeoJSON FeatureCollection in `public/geo/indonesia-provinces.json`.

---

## File Structure

- Modify: `investra-gov-apps-fe/public/geo/indonesia-provinces.json`
  - Responsibility: static 38-province Indonesia GeoJSON used by the map.
- Modify: `investra-gov-apps-fe/src/components/organisms/charts/InteractiveMap.tsx`
  - Responsibility: fetch cluster data, join it to GeoJSON province features, render the interactive choropleth, tooltip, legend, and stats.

---

### Task 1: Validate GeoJSON asset

**Files:**
- Modify: `investra-gov-apps-fe/public/geo/indonesia-provinces.json`

- [ ] **Step 1: Validate feature count and province property names**
...
python - <<'PY'
import json
path = 'investra-gov-apps-fe/public/geo/indonesia-provinces.json'
with open(path, encoding='utf-8') as f:
    data = json.load(f)
features = data.get('features', [])
names = [feature.get('properties', {}).get('PROVINSI') for feature in features]
print(data.get('type'))
print(len(features))
...
The printed names must include:
...
- [ ] **Step 2: Commit GeoJSON asset change**

Run only if commits are requested by the user:

```bash
git add investra-gov-apps-fe/public/geo/indonesia-provinces.json
git commit -m "Add 38-province Indonesia GeoJSON"
```

---

### Task 2: Rewrite InteractiveMap with react-simple-maps

**Files:**
- Modify: `investra-gov-apps-fe/src/components/organisms/charts/InteractiveMap.tsx`

- [ ] **Step 1: Replace the file with the react-simple-maps implementation**

Use this complete content for `InteractiveMap.tsx`:
...
export const InteractiveMap = memo(() => {
  ...
  const provinceByName = useMemo(() => {
    return new Map(provinces.map((province) => [normalizeProvinceName(province.name).toLowerCase(), province]));
  }, [provinces]);
                ...
                geographies.map((geo) => {
                  ...
                  return (
                    <Geography
                      ...
                      aria-label={province ? `${province.name}, ${CLUSTER_LABELS[province.cluster] ?? ""}` : provinceName}
                      onKeyDown={(e) => {
                        if (province && (e.key === "Enter" || e.key === " ")) handleProvinceHover(province);
                      }}
                      style={{
                        default: { outline: "none", opacity: province ? 0.9 : 0.45 },
                        hover: { outline: "none", opacity: 1, stroke: "#002C5F", strokeWidth: 0.75 },
...
```bash
cd investra-gov-apps-fe && npm run build
```
...
If TypeScript rejects `Geographies` callback types, use the existing `@types/react-simple-maps` types or loosen only the callback parameter shape locally:

```tsx
{({ geographies }: { geographies: GeographyFeature[] }) => ...}
```

Expected: no TypeScript errors remain.

- [ ] **Step 4: Commit map rewrite**

Run only if commits are requested by the user:

```bash
git add investra-gov-apps-fe/src/components/organisms/charts/InteractiveMap.tsx
git commit -m "Render dashboard map from 38-province GeoJSON"
```
...
**Files:**
- Verify: `investra-gov-apps-fe/src/components/organisms/charts/InteractiveMap.tsx`
...
```bash
cd investra-gov-apps-fe && npm run dev
```
...
Use the browser to open the Vite URL and navigate to the dashboard route that contains `InteractiveMap`.

Expected:

```text
The map renders with province-shaped polygons, not approximate island silhouettes or dots.
```
...
```text
Zoom in changes the displayed percentage and enlarges the map.
Zoom out reduces the displayed percentage and is disabled at 100%.
Reset returns the displayed percentage to 100%.
Hovering a colored province shows the existing tooltip with province name, cluster, PDRB/Kapita, IPM, and investasi.
The legend shows only active cluster IDs.
The stat cards still count provinces per cluster.
```
...
- Spec coverage: Tasks validate the 38-province GeoJSON, rewrite `InteractiveMap.tsx`, preserve existing data flow/UI, and verify build/manual behavior.
- Placeholder scan: No TBD/TODO/implement-later placeholders are present.
- Type consistency: `Province`, `GeographyFeature`, `normalizeProvinceName`, and `provinceByName` are defined before use and used consistently.
